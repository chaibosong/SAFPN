...
def make_anchor_generator(config):
    ...
    if config.MODEL.RPN.USE_FPN:  # True
...
def generate_anchors(
    ...
    return _generate_anchors(
        stride,  # (4,8,16,32,64)
        np.array(sizes, dtype=np.float) / stride,  # (32/4, 64/8,128/16,256/32,512/64) = (8,8,8,8,8)
        np.array(aspect_ratios, dtype=np.float),  # [0.5,1.0,2.0)
    )
